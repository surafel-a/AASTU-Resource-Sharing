...
const DataRow = ({ fileIcon, fileName, fileSize, fileType, category, dateUploaded, status }) => {
  const statusStyles = {
  ...
  return (
    <>
      <div className="flex items-center gap-4 p-6">
        <div>
          <FontAwesomeIcon icon={fileIcon} className={`text-3xl rounded-xl p-3 ${fileTypeStyles[fileType]}`} />
        </div>
        <div>
      ...
      <div className="p-6">
        <p className={`inline font-bold text-xl px-4 py-1 rounded-md ${statusStyles[status]}`}>
